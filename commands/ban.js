...
module.exports.run = async (bot, message, args) => {
	...
	if (message.member.permissions.has('BAN_MEMBERS')) {
		...
		if (!member.bannable)
			return message.channel.send('Eu não consigo banir este usuário!');
		if (member.user.id === 'seu_id')
			return message.channel.send('Eu não consigo banir o dono!');

		if (member.id === message.author.id)
			return message.channel.send('Você não pode banir a si mesmo!');
		...
		await member
			.ban(reason)
			.catch(error =>
				message.channel.send(`Desculpe eu não consegui por causa de: ${error}`)
			);
		...
		message.channel.send(
			'O Usuário <@' +
				member.id +
				'> foi banido! Esse bobão achou que podia quebrar as regras e estava muito enganado!'
		);
